...
func (message *Message) Decode(data []byte) error {
    ...
    realHeaderLen := MessageHeaderSize + header.UidLen

    entity, _, err := message.decodeBody(MsgID(header.MsgId), data[realHeaderLen:])
    if err == nil {
        message.Body = entity
    } else {
...
func (message *Message) decodeBody(typ MsgID, data []byte) (Entity, int, error) {
